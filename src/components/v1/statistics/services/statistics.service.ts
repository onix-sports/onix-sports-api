    ...
    public async updateGamesProfileStats() {
        ...
        const promises = users.map(({ _id }) => {
            return this.gameService
                ...
                .then((games) => {
                    const sortedGames = [...games].sort((a, b) => b.duration - a.duration);
                    const longestGame = sortedGames[0];
                    const shortestGame = sortedGames[sortedGames.length - 1];

                    return this.profileStatisticsRepository.setGames(
                        _id,
                        {
                            longestGame: longestGame?._id,
                            shortestGame: shortestGame?._id,
                            lastGames: games.map(({ _id }) => _id).slice(-9),
                        },
                    );
                });
        });
